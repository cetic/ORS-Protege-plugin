...
public class JavaCodeGeneratorPanel extends JPanel {
    ...
    public JavaCodeGeneratorPanel(CodeGenerationOptions options,
        ...
        if (options.getNamespace() != null) {
            ...
            String prefix=PrefixUtilities.getPrefixOWLOntologyFormat(generateCodeWithOptions.getOWLModelManager()).getDefaultPrefix();
            namespaceTextField.setText(prefix);
        }
        ...
        JPanel namespaceJPanel = new JPanel(new BorderLayout());
        namespaceJPanel.add(new JLabel("Default namespace"), BorderLayout.WEST);
        add(namespaceJPanel);
        add(getComponentWithNonStretchingVertically(namespaceTextField));
        add(Box.createVerticalStrut(8));
        ...
        okButton.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                options.setPackage(packageTextField.getText().trim().length() > 0 ? packageTextField.getText().trim()
                        : null);
                options.setFactoryClassName(factoryClassNameTextField.getText().trim().length() > 0 ? factoryClassNameTextField
                                .getText().trim()
                                : Constants.FACTORY_CLASS_NAME);
                options.setNamespace(namespaceTextField.getText().trim().length() > 0 ? namespaceTextField
                                .getText().trim()
                                : "");
    ...
    public void ok() {
        ...
        options.setNamespace(namespaceTextField.getText());
